			...
			<div className="mx-auto max-w-4xl">
				...
				<div className="mt-10 grid gap-3 sm:grid-cols-2">
					{report.components.map((component) => (
						<div
							className="flex min-h-36 flex-col justify-between gap-5 rounded-3xl bg-muted/40 p-5"
							key={component.key}
						>
				...
				<div className="mt-6 flex flex-col gap-4 rounded-3xl bg-primary/5 p-5 sm:flex-row sm:items-center sm:justify-between">
					<div className="flex items-start gap-3">
						<Info className="mt-0.5 size-5 shrink-0 text-primary" />
						<p className="max-w-2xl text-muted-foreground text-sm leading-6">
							{m.public_status_customer_notice()}
						</p>
					</div>
					<Button
						className="shrink-0"
...
function componentName(key: HealthComponent["key"]) {
	return {
		...
		commerce_queue: m.public_status_commerce_queue(),
