...
describe("audit log R2 export", () => {
	...
	beforeAll(async () => {
		miniflare = new Miniflare({
			...
			d1Databases: { DB: "gmshop-edge-audit-export" },
		});
	...
	it("writes a bounded, redacted NDJSON artifact", async () => {
		...
		const registry = await db
			.prepare(
				`SELECT artifact_object_key AS object_key, requested_by AS exported_by,
				 record_count, delete_after, artifact_deleted_at AS deleted_at
				 FROM operation_task_runs WHERE artifact_object_key = ?`,
			)
			.bind(result.key)
			.first<{
				object_key: string;
